...
const app = new Hono()
  ...
  .post(
    ...
    async (c) => {
      ...
      const highesPositionTask = await databases.listDocuments(
        DATABASE_ID,
        TASKS_ID,
        [
      ...
      const newPosition =
        highesPositionTask.documents.length > 0
          ? highesPositionTask.documents[0].position + 1000
          : 1000;
  ...
  .post(
    ...
    async (c) => {
      ...
      if (!workspaceId) {
        return c.json({ error: "Workspace is ID required" }, 400);
      }
